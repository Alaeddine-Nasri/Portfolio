...
<template>
  <nav class="sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-white/65 dark:supports-[backdrop-filter]:bg-neutral-950/65 border-b border-neutral-200 dark:border-neutral-800">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
      ...
      <div class="hidden md:flex items-center gap-2">
        <RouterLink
          v-for="l in links" :key="l.label" :to="l.to"
          class="px-3 py-1.5 rounded-lg text-sm hover:bg-neutral-100 dark:hover:bg-neutral-800"
          :class="isActive(l.to.name as string) ? 'bg-neutral-200 dark:bg-neutral-800 font-medium' : ''"
        >{{ l.label }}</RouterLink>
        <DarkToggle />
      </div>

      <button class="md:hidden rounded-lg border px-2 py-1" @click="open = !open" aria-label="Toggle menu">
        ☰
      </button>
    </div>
    ...
    <div v-if="open" class="md:hidden border-t border-neutral-200 dark:border-neutral-800">
      <div class="max-w-5xl mx-auto px-4 py-2 flex flex-col gap-1">
        <RouterLink
          v-for="l in links" :key="l.label" :to="l.to" @click="open=false"
